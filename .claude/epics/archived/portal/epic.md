...
## Tasks Created
- [x] #2 - Next.js スキャフォールド + 静的エクスポート設定 + 共通レイアウト (parallel: true)
- [x] #3 - ダミーデータ一式 + 型定義 (src/data) (parallel: true)
- [x] #4 - ダッシュボードページ (app/page.tsx) (parallel: true, depends: #2,#3)
- [x] #5 - 部署一覧ページ (app/departments/page.tsx) (parallel: true, depends: #2,#3)
- [x] #6 - 社員名簿ページ — 検索 + 部署フィルタ (parallel: true, depends: #2,#3)
- [x] #7 - FAQ・申請ガイドページ (app/faq/page.tsx) (parallel: true, depends: #2,#3)
- [x] #8 - GitHub Actions による GitHub Pages デプロイ (parallel: true, depends: #2)
- [x] #9 - README・会社設定ドキュメント (parallel: true)
